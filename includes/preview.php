...
class Mintboard_Editor_Preview {
	...
	function choose_template() {
		...
		if ( get_post_type( $p ) !== Mintboard_Editor::post_type ) return;
		...
		$template = false;
		if     (Mintboard_Editor::is_media_fork($p) && $template = get_attachment_template()     ) :
			remove_filter('the_content', 'prepend_attachment');
		elseif ( $t->is_404()            && $template = get_404_template()            ) :
