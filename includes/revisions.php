...
class Mintboard_Editor_Revisions {
	...
	function get_previous_post_revision( $p = null ) {
		...
		if ( $p->post_type == Mintboard_Editor::post_type ) {
			_doing_it_wrong('get_previous_post_revision', 'Function applies to posts, not forks', null);
			return false;
		}
	...
	function get_previous_revision( $p = null ) {
		...
		if ( $p->post_type != Mintboard_Editor::post_type ) {
			_doing_it_wrong('get_previous_revision', 'Function only applies to forks, not posts', null);
			return false;
		}
	...
	function get_parent_revision( $p = null ) {
		...
		if ( $p->post_type != Mintboard_Editor::post_type ) {
			_doing_it_wrong( 'get_parent_revision', 'Function only applies to forks, not posts', null );
			return false;
		}
